...
export const RigidShapeCollison = {
    clamp,
    collidedRectCirc,
...
function collidedRectCirc(rect1Shape, circ2Shape, collisionInfo) {
...
function resolveParticleCollision(aParticle) {
    var status = false;
    switch (this.rigidType()) {
        case RigidShape.eRigidType.eRigidCircle:
            status = this.gEngine.Particle.resolveCirclePos(this, aParticle);
            break;
        case RigidShape.eRigidType.eRigidRectangle:
            status = this.gEngine.Particle.resolveRectPos(this, aParticle);
            break;
    }
    return status;
};